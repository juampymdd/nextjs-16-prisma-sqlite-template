...
export const KanbanColumn = ({ column, tasks }: Props) => {
  ...
  return (
    <div className="flex flex-col gap-4 min-w-[320px] max-w-[320px]">
      <div className="flex items-center justify-between px-4 py-2 bg-muted/20 rounded-2xl border border-border/5">
        ...
        <DialogContent className="sm:max-w-[425px] rounded-[2rem]">
          <DialogHeader>
            ...
            <DialogDescription className="py-2">
              Esta acción eliminará el tablero <strong>"{column.name}"</strong> y moverá todas las tareas asociadas al tablero anterior disponible.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="flex gap-2 sm:justify-end">
            <Button variant="ghost" onClick={() => setIsConfirmOpen(false)} className="rounded-xl">
              Cancelar
            </Button>
      ...
      <div
        ref={setNodeRef}
        className="flex-1 bg-muted/40 rounded-[2rem] p-4 min-h-[600px] border border-border/10 flex flex-col gap-4 backdrop-blur-sm shadow-inner"
      >
        <SortableContext
